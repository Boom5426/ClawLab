...
`init -> [optional ingest-cv] -> company init -> project create -> task run -> learn -> status`
...
2. `ingest-cv`（可选但推荐）
   - 读取 CV 文本或 PDF
   - 创建或更新 `ResearcherProfile`
3. `company init`
   - 读取已有 `ResearcherProfile`
   - 若还没有 profile，则通过最小 founder intake 创建基础档案
   - 生成 founder / company / team 配置
   - 可顺手创建第一个 active project
4. `project create`
